...
- **按键**：电源键短按=启动/停止测量，长按 3s=关机（保存计数）；模式键单击=单次→连续→多功能→测试（600ms 窗口），三击=计数清零（**立即写 Flash**），四击=HEr，五/六击=磁场校准开始/结束，七击=PIt（双键 1s 切 HIt，再按保存退出），八击=加计校准，九击=角度参考，**十击=JY901B 恢复出厂并重新写入垂直安装/5Hz角度帧配置**；页内电源+/模式− 0.1° 步进（800ms 后 100ms 连发）
