...
/*
 * 硬件事实（已由图纸和实测确认）：
 *   每组特殊段只能显示 1/2/3，无法显示 0（千位为 0 时熄灭）。
 *   每组 4 段的物理构成完全一致：
 *     [0] 三横连通段（上横 + 中横 + 下横，在硬件上内部串联为同一段）
 *     [1] 右上竖
 *     [2] 左下竖
 *     [3] 右下竖
 *
 *   各区域对应 S 编号：
 *     - 方位角百位 (LCD_HISEG_HEADING)：S26三横, S25右上, S27左下, S28右下
 *     - 距离千位   (LCD_HISEG_DIST)   ：S9三横,  S11右上, S8左下,  S10右下
 *     - 高程千位   (LCD_HISEG_ELEV)   ：S44三横, S43右上, S46左下, S47右下
 *
 *   笔画组合字形（bit0=三横, bit1=右上, bit2=左下, bit3=右下）：
 *     - 显示 0：0x00（全灭）
 *     - 显示 1：右上(bit1) + 右下(bit3)                 = 0x0A
 *     - 显示 2：三横(bit0) + 右上(bit1) + 左下(bit2)     = 0x07
 *     - 显示 3：三横(bit0) + 右上(bit1) + 右下(bit3)     = 0x0B
 */
static const uint8_t lcd_hiseg_strokes[3][4] = {
    [LCD_HISEG_HEADING] = {26, 25, 27, 28}, /* S26三横, S25右上, S27左下, S28右下 */
    [LCD_HISEG_DIST]    = {9,  11, 8,  10}, /* S9三横,  S11右上, S8左下,  S10右下 */
    [LCD_HISEG_ELEV]    = {44, 43, 46, 47}, /* S44三横, S43右上, S46左下, S47右下 */
};

static const uint8_t lcd_hiseg_font[4] = {
    0x00U, /* 0: 无法显示，熄灭 */
    0x0AU, /* 1: 右上 + 右下 (如 S11 + S10) */
    0x07U, /* 2: 三横 + 右上 + 左下 (如 S9 + S11 + S8) */
    0x0BU, /* 3: 三横 + 右上 + 右下 (如 S9 + S11 + S10) */
};
